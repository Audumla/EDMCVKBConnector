...
Usage:
    python scripts/agent_runners/run_agent_plan.py \
        --planner gemini \
        --executor codex \
...
CONFIG_FILE = PROJECT_ROOT / "scripts" / "agent_runners" / "delegation-config.json"
_config_defaults = {}
_planners_config = {}
_executors_config = {}
...
def get_planner_defaults(planner: str) -> dict[str, Any]:
    return _planners_config.get(planner, {})
...
def run_executor(executor: str, plan_file: Path, thinking_budget: str, extra_args: list[str]) -> tuple[int, str | None]:
    ...
    cmd = [
        sys.executable,
        str(PROJECT_ROOT / "scripts" / "agent_runners" / runner_script),
    ...
    if "bin" in executor_cfg:
        # Check if --codex-bin is already in extra_args to avoid duplication
        if not any(arg == "--codex-bin" for arg in extra_args):
            cmd.extend(["--codex-bin", executor_cfg["bin"]])
