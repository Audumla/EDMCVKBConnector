...
- Follow machine-local `AGENT.md` if it exists. That file is generated by the
  external runner and contains the correct artifact paths and commands.
- Do not assume local runner scripts such as `scripts/agent_runners/*` exist.
- Keep any machine-local artifacts out of the tracked source tree.
- Prefer edits in `src/edmcruleengine/`, `load.py`, `docs/`, `scripts/`, and
  `test/`.
- Validate changes with the relevant plugin test coverage before finishing.
...
Resolved at install time in `.vscode/agent-system.json` (not committed).
The wrapper scripts in `agent_runner/scripts/` read this file automatically.
<!-- agent-runner-end -->
